...
const validateInput = () => {
    name.value = name.value.replace(/[^а-яА-ЯёЁ\s\-]/g, '').split(' ').map(word => {
...
const isIconHidden = computed(() => {
    return (isError.value || (errors.value !== undefined) || (name.value === undefined))
})
    ...
    <div class="input-form">
        <div :class="['input-field', { 'errorBackligth': isError }]" ref="inputForm"
            ...
            <div class="input-wrapper">
                ...
                <input ref="inputField" v-model="name" v-bind="nameAttrs" :placeholder="constants.NAME_PLACEHOLDER"
                    @input="validateInput()" @focus="changeBackligth"
                    :class="['input', { 'isHidden': !isInputVisible }]" />
            </div>
